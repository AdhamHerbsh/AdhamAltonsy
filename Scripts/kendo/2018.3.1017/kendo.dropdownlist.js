  ...
  function () {
    ...
    (function ($, undefined) {
      var kendo = window.kendo,
        ...
        STATE_FILTER = "filter",
        STATE_ACCEPT = "accept",
        MSG_INVALID_OPTION_LABEL =
          "The `optionLabel` option is not valid due to missing fields. Define a custom optionLabel as shown here http://docs.telerik.com/kendo-ui/api/javascript/ui/dropdownlist#configuration-optionLabel",
        proxy = $.proxy;
      var DropDownList = Select.extend({
        init: function (element, options) {
          ...
          element = that.element.on(
            "focus" + ns,
            proxy(that._focusHandler, that)
          );
        ...
        open: function () {
          var that = this;
          var isFiltered = that.dataSource.filter()
            ? that.dataSource.filter().filters.length > 0
            : false;
        ...
        _allowOpening: function () {
          return (
            this.hasOptionLabel() ||
            this.filterInput ||
            Select.fn._allowOpening.call(this)
          );
        },
        ...
        _optionLabel: function () {
          ...
          if (!that.hasOptionLabel()) {
            that.optionLabel = $(
              '<div class="k-list-optionlabel"></div>'
            ).prependTo(that.list);
          }
          that.optionLabel
            .html(template(optionLabel))
            .off()
        ...
        _optionLabelText: function () {
          ...
          return typeof optionLabel === "string"
            ? optionLabel
            : this._text(optionLabel);
        },
        _optionLabelDataItem: function () {
          var that = this;
          var optionLabel = that.options.optionLabel;
          if (that.hasOptionLabel()) {
            return $.isPlainObject(optionLabel)
              ? new ObservableObject(optionLabel)
        ...
        _attachFocusHandlers: function () {
          ...
          wrapper
            .on("focusin" + nsFocusEvent, proxy(that._focusinHandler, that))
            .on("focusout" + nsFocusEvent, proxy(that._focusoutHandler, that));
          if (that.filterInput) {
            that.filterInput
              .on("focusin" + nsFocusEvent, proxy(that._focusinHandler, that))
              .on(
                "focusout" + nsFocusEvent,
                proxy(that._focusoutHandler, that)
              );
          }
        },
        ...
        _editable: function (options) {
          ...
          if (!readonly && !disable) {
            ...
            wrapper
              ...
              .on(
                kendo.support.mousedown + ns,
                proxy(that._wrapperMousedown, that)
              )
        ...
        _selectNext: function () {
          ...
          if (idx !== dataLength) {
            ...
            that
              ._select(normalizeIndex(startIndex + idx, dataLength))
              .done(function () {
                var done = function () {
                  if (!that.popup.visible()) {
                    that._change();
        ...
        _getElementDataItem: function (element) {
          ...
          return this.listView.dataItemByIndex(
            this.listView.getElementIndex(element)
          );
        },
        _click: function (e) {
          var that = this;
        ...
        _focusElement: function (element) {
          ...
          var touchEnabled =
            support.mobileOS &&
            (support.touch || support.MSPointers || support.pointers);
          if (filterInput && filterInput[0] === element[0] && touchEnabled) {
            return;
          }
          if (
            filterInput &&
            (compareElement[0] === active || this._focusFilter)
          ) {
        ...
        _focusItem: function () {
          ...
          if (index !== undefined) {
            ...
            if (
              options.optionLabel &&
              (!options.virtual || options.virtual.mapValueTo !== "dataItem")
            ) {
              this._focus(this.optionLabel);
        ...
        _resetOptionLabel: function (additionalClass) {
          this.optionLabel
            .removeClass("k-state-focused" + (additionalClass || ""))
            .removeAttr("id");
        },
        ...
        _mobile: function () {
          ...
          if (root.length && mobileOS) {
            popup.options.animation.open.effects =
              mobileOS.android || mobileOS.meego
                ? "fadeIn"
                : mobileOS.ios || mobileOS.wp
                ? "slideIn:up"
                : popup.options.animation.open.effects;
          }
        },
        _filterHeader: function () {
          ...
          if (this._isFilterEnabled()) {
            ...
            this.list.prepend(
              $('<span class="k-list-filter" />').append(
                this.filterInput.add(icon)
              )
            );
          }
        },
        ...
        _inputTemplate: function () {
          ...
          if (!template) {
            template = $.proxy(
              kendo.template("#:this._text(data)#", { useWithBlock: false }),
              that
            );
          } else {
            template = kendo.template(template);
        ...
        _textAccessor: function (text) {
          ...
          if (this.hasOptionLabel()) {
            if (
              ...
              if (
                typeof this.options.optionLabel === "string" &&
                !this.options.optionLabelTemplate
              ) {
                dataItem = optionLabelText;
              }
            }
