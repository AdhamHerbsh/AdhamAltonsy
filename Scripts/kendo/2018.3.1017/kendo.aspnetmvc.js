  ...
  function () {
    (function ($, undefined) {
      ...
      function parameterMap(options, operation, serializationOptions) {
        var result = {};
        if (options.sort) {
          result[this.options.prefix + "sort"] = $.map(
            options.sort,
            function (sort) {
        ...
        if (options.group) {
          result[this.options.prefix + "group"] = $.map(
            options.group,
            function (group) {
        ...
        if (options.aggregate) {
          result[this.options.prefix + "aggregate"] = $.map(
            options.aggregate,
            function (aggregate) {
        ...
        if (options.filter) {
          result[this.options.prefix + "filter"] = serializeFilter(
            options.filter,
            serializationOptions.encode
          );
      ...
      Serializer.prototype = Serializer.fn = {
        ...
        serializeArray: function (result, data, prefix) {
          ...
          for (
            var sourceIndex = 0, destinationIndex = 0;
      ...
      function serializeFilter(filter, encode) {
        ...
        if (filter.field) {
          return (
            ...
            filter.operator +
            "~" +
            encodeFilterValue(filter.value, encode)
          );
        } else {
          return undefined;
        }
      ...
      function encodeFilterValue(value, encode) {
        if (typeof value === "string") {
          if (value.indexOf("Date(") > -1) {
            value = new Date(
              parseInt(value.replace(/^\/Date\((.*?)\)\/$/, "$1"), 10)
            );
          } else {
        ...
        if (value && value.getTime) {
          return (
            "datetime'" + kendo.format("{0:yyyy-MM-ddTHH-mm-ss}", value) + "'"
          );
        }
        return value;
      }
      ...
      function translateAggregate(aggregates) {
        ...
        for (key in aggregates) {
          ...
          for (functionName in aggregate) {
            functionResult[functionName.toLowerCase()] =
      ...
      function convertAggregates(aggregates) {
        ...
        for (idx = 0, length = aggregates.length; idx < length; idx++) {
          ...
          result[aggregate.Member] = extend(
            true,
            result[aggregate.Member],
            translateAggregateResults(aggregate)
          );
        }
        return result;
      }
        ...
        transports: {
          "aspnetmvc-ajax": kendo.data.RemoteTransport.extend({
            init: function (options) {
              ...
              kendo.data.RemoteTransport.fn.init.call(
                this,
      ...
      extend(true, kendo.data, {
        schemas: { webapi: kendo.data.schemas["aspnetmvc-ajax"] },
      });
      extend(true, kendo.data, {
        transports: {
...
(function (f, define) {
  define("aspnetmvc/kendo.combobox.aspnetmvc", [
...
(function (f, define) {
  define("aspnetmvc/kendo.multicolumncombobox.aspnetmvc", [
    "aspnetmvc/kendo.data.aspnetmvc",
  ], f);
})(
  function () {
    (function ($, undefined) {
      ...
      if (ui && ui.MultiColumnComboBox) {
        ui.MultiColumnComboBox.requestData = function (selector) {
          var multicolumncombobox = $(selector).data(
...
(function (f, define) {
  define("aspnetmvc/kendo.dropdownlist.aspnetmvc", [
    "aspnetmvc/kendo.data.aspnetmvc",
  ], f);
})(
  function () {
...
(function (f, define) {
  define("aspnetmvc/kendo.dropdowntree.aspnetmvc", [
    "aspnetmvc/kendo.data.aspnetmvc",
  ], f);
})(
  function () {
...
(function (f, define) {
  define("aspnetmvc/kendo.multiselect.aspnetmvc", [
...
(function (f, define) {
  define("aspnetmvc/kendo.imagebrowser.aspnetmvc", [
  ...
  function () {
    (function ($, undefined) {
      ...
      extend(true, kendo.data, {
        transports: {
          "filebrowser-aspnetmvc":
            kendo.data.transports["imagebrowser-aspnetmvc"],
        },
      });
    })(window.kendo.jQuery);
  },
...
(function (f, define) {
  define("aspnetmvc/kendo.validator.aspnetmvc", [
    ...
    (function ($, undefined) {
      ...
      function rulesForField(field) {
        ...
        for (idx = 0, length = fieldRules.length; idx < length; idx++) {
          ...
          rules[fieldName + validationType] = createMetaRule(
            fieldName,
            validationType,
            validationParams
          );
          messages[fieldName + validationType] = createMetaMessage(
      ...
      var validationRules = {
        ...
        number: function (input) {
          return (
            input.val() === "" ||
            input.val() == null ||
            kendo.parseFloat(input.val()) !== null
          );
        },
        ...
        length: function (input, params) {
          if (input.val() !== "") {
            ...
            return (
              (!params.min || len >= (params.min || 0)) &&
              (!params.max || len <= (params.max || 0))
            );
          }
          return true;
        },
        ...
        messageLocators: {
          mvcLocator: {
            locate: function (element, fieldName) {
              ...
              return element.find(
                ".field-validation-valid[data-valmsg-for='" +
                  fieldName +
          ...
          mvcMetadataLocator: {
            locate: function (element, fieldName) {
              fieldName = fieldName.replace(nameSpecialCharRegExp, "\\$1");
              return element.find(
                "#" + fieldName + "_validationMessage.field-validation-valid"
              );
            },
            decorate: function (message, fieldName) {
              message
                .addClass("field-validation-error")
                .attr("id", fieldName + "_validationMessage");
            },
          },
        },
        ruleResolvers: {
